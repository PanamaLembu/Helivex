...
if ( ! empty( $settings_option['delete_data_on_uninstall'] ) && '1' === $settings_option['delete_data_on_uninstall'] ) {
    ...
    if ( ! empty( $settings_option['enable_cloudflare'] ) && '1' === $settings_option['enable_cloudflare'] &&
        ...
        $cf_token = $settings_option['cf_api_token'];
        $cf_zone  = $settings_option['cf_zone_id'];
        ...
        while ($has_more) {
            $res = advaipbl_uninstall_cf_req('GET', "zones/{$cf_zone}/firewall/access_rules/rules?per_page=100&page={$page}", $cf_token);
            ...
            foreach ($res['result'] as $rule) {
                if (isset($rule['notes']) && strpos($rule['notes'], '[AIB]') !== false) {
                    $rules_to_delete[] = $rule['id'];
                }
            }

            $total_pages = $res['result_info']['total_pages'] ?? 1;
        ...
        foreach ($rules_to_delete as $rule_id) {
            advaipbl_uninstall_cf_req('DELETE', "zones/{$cf_zone}/firewall/access_rules/rules/{$rule_id}", $cf_token);
        }
    }
    ...
    if ( file_exists( $htaccess_path ) && is_writable( $htaccess_path ) ) {
        ...
        if ($content !== false) {
            ...
            $new_content = preg_replace('/# BEGIN Advanced IP Blocker.*?# END Advanced IP Blocker\s*/s', '', $content);
