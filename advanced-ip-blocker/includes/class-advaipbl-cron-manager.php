...
class ADVAIPBL_Cron_Manager {
    ...
    public function add_cron_intervals( $schedules ) {
        ...
        $schedules['three_days'] = [
            'interval' => 259200, // 3 days * 24 * 60 * 60
            'display'  => __( 'Every 3 Days', 'advanced-ip-blocker' )
        ];
        $schedules['weekly'] = [
            'interval' => 604800,
    ...
    public function schedule_jobs() {
        ...
        // If it was scheduled 'weekly' before, we need to clear it to apply the new 'three_days' schedule.
        // WP doesn't auto-update the interval if the hook name is the same.
        ...
        if (!wp_next_scheduled('advaipbl_update_geoip_db_event')) {
            wp_schedule_event(time() + wp_rand(0, 12 * HOUR_IN_SECONDS), 'three_days', 'advaipbl_update_geoip_db_event');
        }
