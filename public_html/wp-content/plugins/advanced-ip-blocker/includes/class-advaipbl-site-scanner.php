...
class ADVAIPBL_Site_Scanner {
    ...
    public function run_local_scan() {
        return [
            ...
            'wordpress'     => $this->check_wp_version(),
            'database'      => $this->check_db_version(),
            'debug_mode'    => $this->check_debug_mode(),
            'updates'       => $this->check_pending_updates(),
            'ssl'           => $this->check_ssl(),
            'themes_list'   => $this->get_themes_status_list(), // New: Detailed theme list
        ];
    }
    ...
    public function check_vulnerabilities_via_api() {
        ...
        $api_url = 'https://advaipbl.com/wp-json/aib-scanner/v1/check';
    ...
    public function check_server_reputation() {
        ...
        $api_check_url = 'https://advaipbl.com/wp-json/aib-scanner/v1/check-ip?ip=' . $server_ip;
        $response = wp_remote_get($api_check_url, ['timeout' => 5]);
    ...
    public function run_full_scan_and_email($to_email, $is_manual = false) {
        ...
        if ( ! $is_manual ) {
            ...
            // Check Updates
            if ($local_scan['updates']['status'] !== 'good') $is_clean = false;
            
            // Check Environment Health
            if ($local_scan['php']['status'] !== 'good') $is_clean = false;
            if ($local_scan['wordpress']['status'] !== 'good') $is_clean = false;
            if ($local_scan['ssl']['status'] !== 'good') $is_clean = false;
            if ($local_scan['debug_mode']['status'] !== 'good') $is_clean = false;
