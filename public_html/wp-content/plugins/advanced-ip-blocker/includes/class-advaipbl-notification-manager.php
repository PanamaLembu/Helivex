...
class ADVAIPBL_Notification_Manager {
    ...
    public function send_summary_email() {
        ...
        $notifications = $wpdb->get_results("SELECT * FROM $table_name ORDER BY timestamp ASC");
        ...
        /* translators: 1: Site name, 2: Total number of blocked IPs. */
        $email_subject = sprintf(__('[%1$s] Security Summary: %2$d New Blocks', 'advanced-ip-blocker'), $site_name, $total_blocks);
        /* translators: %d: Total number of blocked IPs. */
        $template_title = sprintf( _n( '%d New Block', '%d New Blocks', $total_blocks, 'advanced-ip-blocker' ), $total_blocks );

        // 1. Construir el contenido HTML
        $blocks_by_type = [];
        foreach ($notifications as $notification) {
            $blocks_by_type[$notification->block_type][] = $notification;
        }
        
        $summary_html = '<p style="font-size: 16px; line-height: 1.6;">' . esc_html__( 'Here is your security summary for the last period.', 'advanced-ip-blocker' ) . '</p>';
        ...
        foreach ( $blocks_by_type as $type => $blocks ) {
            ...
            /* translators: 1: Block type (e.g. Honeypot, 404). 2: Number of blocks. */
            $summary_html .= '<li><strong>' . esc_html( $type_capitalized ) . ':</strong> ' . sprintf( esc_html( _n( '%d block', '%d blocks', $block_count, 'advanced-ip-blocker' ) ), $block_count ) . '</li>';
        }
        $summary_html .= '</ul>';
        
        $summary_html .= '<h3 style="margin-top: 25px; margin-bottom: 10px; border-bottom: 1px solid #eee; padding-bottom: 5px;">' . esc_html__( 'Recent Block Details', 'advanced-ip-blocker' ) . '</h3>';
        $summary_html .= '<table style="width: 100%; border-collapse: collapse;"><thead><tr style="background-color: #f9f9f9;">' .
                        ...
                        '<th style="padding: 8px; border: 1px solid #ddd; text-align: left;">' . esc_html__('Blocked IP', 'advanced-ip-blocker') . '</th>' .
        ...
        foreach ( $limited_notifications as $notification ) {
            ...
            $summary_html .= '<tr>' .
                            ...
                            '<td style="padding: 8px; border: 1px solid #ddd;">' . esc_html( $notification->ip ) . '</td>' .
        ...
        $summary_html .= '<table style="width: 100%; text-align: center; margin-top: 30px;"><tr><td>' .
                        '<a href="' . esc_url( $button_url ) . '" style="' . esc_attr( $button_style ) . '">' . esc_html__( 'Manage All Blocked IPs', 'advanced-ip-blocker' ) . '</a>' .
                        '</td></tr></table>';

        // 2. Envolver en la plantilla y enviar
        ...
        if ($sent) {
            ...
            $this->plugin->log_event( sprintf( __( 'Security summary sent to %s and queue cleared.', 'advanced-ip-blocker' ), $to ), 'info' );
        } else {
            /* translators: %s: The recipient's email address. */
            $this->plugin->log_event( sprintf( __( 'FAILED to send security summary to %s. Queue NOT cleared.', 'advanced-ip-blocker' ), $to ), 'error' );
        }
    }
    ...
    public function notify_block($ip, $type, $reason, $reason_label = '', $extra_data = []) {
        ...
        if (!empty($options['enable_push_notifications'])) {
            ...
            if ($send_this_push) {
                ...
                // Logic update: If minutes > 525600 (1 year), treat as Permanent
