...
private function __construct() {
    ...
    if (version_compare(PHP_VERSION, '8.1', '>=')) {
        // El GeoIP Manager solo se instancia si se necesita, pero su archivo ya está cargado.
    ...
    public function log_request_signature() {
        ...
        if (wp_doing_cron() || is_admin() || (defined('WP_CLI') && WP_CLI)) { return; }
        if ($this->challenge_passed_this_request) { return; }
   
        // FIX: Si el bot ya ha sido verificado como legítimo (Google, Bing, etc.), NO lo registramos como firma sospechosa.
        // Esto evita falsos positivos en el log de firmas y permite confiar en que lo que queda aquí son impostores o desconocidos.
...
public function verify_known_bots() {
    ...
    if ($is_verified) {
        ...
        // CRITICAL FIX: Bloqueo INSTANTÁNEO (Zero Tolerance).
        // Saltamos el sistema de puntuación porque la suplantación es una amenaza crítica confirmada.
...
public function get_blocked_endpoints_count() {
    ...
    */
    ...
    public function handle_run_manual_scan() {
        ...
        $active_main_tab = isset($_GET['tab']) ? sanitize_key($_GET['tab']) : 'dashboard';
    ...
    public function handle_login_page_restriction() {
        ...
        if ( ! empty( $this->options['restrict_login_page'] ) ) {
            ...
            if ( ! $this->is_whitelisted( $client_ip ) ) {
                ...
                $this->log_event( sprintf( __( 'Access to wp-login.php denied for non-whitelisted IP: %s', 'advanced-ip-blocker' ), $client_ip ), 'critical', $client_ip );
...
/**
    ...
    public function block_ip_instantly($ip, $type, $reason_message, $extra_data = [], $context = 'frontend_block', $custom_duration_seconds = null) {
        ...
        if (!$wpdb->query($wpdb->prepare("INSERT IGNORE INTO {$wpdb->prefix}advaipbl_cache (cache_key, cache_value, expires_at) VALUES (%s, '1', %d)", $lock_key, time() + 15))) {
...
public function get_ip_intelligence() {
    ...
    public function log_event( $message, $level = 'info', $details = [] ) {
...
public function admin_menu() {
    ...
    add_submenu_page(
        '', // No tener padre de menú la oculta.
        __( 'Setup Wizard', 'advanced-ip-blocker' ),
        __( 'Setup Wizard', 'advanced-ip-blocker' ),
        'manage_options',
        'advaipbl-setup-wizard', // Slug único para nuestra página.
...
private function get_first_public_ip_from_string($ip_string) {
   ...
   public function settings_page_content() {
     ...
     */
        ...
        $endpoint_url = 'https://advaipbl.com/wp-json/telemetry/v1/submit';
...
public function handle_import_settings() {
     ...
     */
        ...
        if ( ! $global_2fa_enabled || ! $this->tfa_manager ) {
            return $user;
        }
...
/**
    ...
    public function execute_community_report() {
        ...
        // 3. Enviar a la API (Endpoint provisional)
        $api_url = 'https://advaipbl.com/wp-json/aib-network/v1/report';
        
        $response = wp_remote_post( $api_url, [
            'body'    => wp_json_encode($payload),
            'headers' => [
                'Content-Type' => 'application/json',
                'X-AIB-Site-Hash' => $payload['site_hash'], // Cabecera de autenticación simple
            ],
            'timeout' => 10,
