...
function helivex_create_research_kit_product() {
    ...
    if ($existing_id) {
        ...
        // Force name update to "10-Vial Kits"
        $product->set_name('10-Vial Kits');
    } else {
    ...
    $custom_peptides = get_option('helivex_kit_peptides', '');
    if (!empty($custom_peptides)) {
        $peptides = array_map('trim', explode("\n", $custom_peptides));
    } else {
        $peptides = [
            'RT 5mg',
            'TRZ 10mg',
...
/**
 * Admin Feature: Manage Kit Options
 */
function helivex_admin_menu() {
    add_menu_page(
        'Kit Options',
        'Kit Options',
        'manage_options',
        'helivex-kit-options',
        'helivex_kit_options_page',
        'dashicons-list-view',
        30
    );
}
add_action('admin_menu', 'helivex_admin_menu');

function helivex_kit_options_page() {
    if (isset($_POST['submit'])) {
        update_option('helivex_kit_peptides', sanitize_textarea_field($_POST['kit_peptides']));
        echo '<div class="updated"><p>Kit options updated! Run the kit creator to apply changes.</p></div>';
    }
    $peptides = get_option('helivex_kit_peptides', '');
    ?>
    <div class="wrap">
        <h1>10-Vial Kit Options</h1>
        <form method="post">
            <p>Enter one peptide per line (e.g., "RT 5mg"):</p>
            <textarea name="kit_peptides" rows="10" cols="50" class="large-text"><?php echo esc_textarea($peptides); ?></textarea>
            <?php submit_button('Save Kit Options'); ?>
        </form>
        <hr>
        <h2>Apply Changes</h2>
        <p>After saving, click below to update the product variations:</p>
        <a href="<?php echo esc_url(add_query_arg('helivex_create_kit', '1', admin_url())); ?>" class="button button-primary">Update Kit Product</a>
    </div>
    <?php
}

/**
 * Add GHK-Cu(Cosmetic Grade) Product
 */
function helivex_create_ghk_product() {
    if (!isset($_GET['helivex_create_ghk']) || $_GET['helivex_create_ghk'] !== '1') {
        return;
    }
    ...
    $product->set_name('GHK-Cu (Cosmetic Grade)');
    $product->set_regular_price('80.00');
    $product->set_description('Premium cosmetic grade GHK-Cu (Copper Peptide) for topical research applications. Available in 2g pure lyophilized format.');
    $product->set_status('publish');
    $product->set_category_ids([get_term_by('name', 'Peptides', 'product_cat')->term_id]);
    ...
    if (function_exists('helivex_set_product_image_from_url')) {
        helivex_set_product_image_from_url($product_id, 'helivex-vial-blue.png');
    }

    echo "GHK-Cu Product Created/Updated successfully! Product ID: $product_id";
    exit;
}
add_action('admin_init', 'helivex_create_ghk_product');

/**
 * Global Product Name & Content Replacements
 */
function helivex_global_replacements($text) {
    if (is_admin() && !defined('DOING_AJAX')) return $text;
...
add_filter('the_title', 'helivex_global_replacements', 20);
add_filter('the_content', 'helivex_global_replacements', 20);
add_filter('woocommerce_product_get_name', 'helivex_global_replacements', 20);
add_filter('woocommerce_product_get_short_description', 'helivex_global_replacements', 20);
add_filter('woocommerce_product_get_description', 'helivex_global_replacements', 20);
add_filter('woocommerce_cart_item_name', 'helivex_global_replacements', 20);

/**
 * Force enable WooCommerce registration
 */
add_filter('option_woocommerce_enable_myaccount_registration', '__return_yes');
add_filter('option_woocommerce_enable_signup_and_login_from_checkout', '__return_yes');
add_filter('option_woocommerce_registration_generate_password', '__return_no'); // Show password field
add_filter('option_woocommerce_registration_generate_username', '__return_no'); // Show username field

// Helper for __return_yes/no if not defined (standard WP is __return_true/false but WC options use yes/no)
function __return_yes() { return 'yes'; }
function __return_no() { return 'no'; }

/**
 * Registration helper for header
 */
function helivex_get_account_link() {
    if (is_user_logged_in()) {
        return '<a href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '" class="btn-primary px-4 py-2 text-[10px]">MY PROTOCOLS</a>';
    } else {
        return '<div class="flex items-center gap-2">
                    <a href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '" class="text-[10px] font-black tracking-widest hover:text-primary transition-colors">LOGIN</a>
                    <span class="text-zinc-300">/</span>
                    <a href="' . esc_url(get_permalink(get_option('woocommerce_myaccount_page_id'))) . '" class="btn-primary px-4 py-2 text-[10px]">CREATE ACCOUNT</a>
                </div>';
    }
}
